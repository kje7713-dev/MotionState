"""Tests for GET /videos/{video_id}/detections."""
...
def _make_detections_payload(video_id: str = "1") -> dict:
    return {
        "video_id": video_id,
        "version": 1,
        "sample_fps": 2.0,
...
@pytest.mark.asyncio
async def test_detections_returns_200(app, tmp_path):
    ...
    payload = _make_detections_payload("1")
    artifact_file = tmp_path / "detections.json"
    ...
    with patch.object(settings, "artifacts_dir", str(tmp_path)):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.get("/videos/1/detections")
...
@pytest.mark.asyncio
async def test_detections_response_includes_expected_fields(app, tmp_path):
    from libs.db import get_db
    from libs.models import Video
...
@pytest.mark.asyncio
async def test_detections_returns_404_for_missing_video(app):
    from libs.db import get_db

    app.dependency_overrides[get_db] = _db_override(video=None)

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get("/videos/9999/detections")
...
@pytest.mark.asyncio
async def test_detections_returns_404_when_artifact_missing(app):
    from libs.db import get_db
    from libs.models import Video
...
@pytest.mark.asyncio
async def test_detections_returns_404_when_file_missing(app, tmp_path):
    from libs.db import get_db
    from libs.models import Video
