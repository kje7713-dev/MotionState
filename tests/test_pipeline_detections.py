...
class TestDetectionsArtifact:
    def test_video_id_in_artifact(self, tmp_path):
        ...
        _, det, _, _, _ = run_pipeline(
            "vid-1", frames=[frame], detector=_OnePerson(), sample_fps=2.0
        )
    ...
    def test_sample_fps_stored(self, tmp_path):
        frame = _make_frame(tmp_path, 0)
        _, det, _, _, _ = run_pipeline(
            "vid-1", frames=[frame], detector=_OnePerson(), sample_fps=5.0
        )
    ...
    def test_frames_list_length_matches_input(self, tmp_path):
        ...
        _, det, _, _, _ = run_pipeline("v", frames=frames, detector=_OnePerson(), sample_fps=2.0)
        assert len(det["frames"]) == 3

    def test_each_frame_entry_has_required_keys(self, tmp_path):
        frame = _make_frame(tmp_path, 0)
        _, det, _, _, _ = run_pipeline("v", frames=[frame], detector=_OnePerson(), sample_fps=2.0)
        entry = det["frames"][0]
...
class TestStateSummary:
    def test_frame_count_matches_input(self, tmp_path):
        ...
        state, _, _, _, _ = run_pipeline("v", frames=frames, detector=_OnePerson(), sample_fps=2.0)
    ...
    def test_total_detections_two_per_frame(self, tmp_path):
        ...
        state, _, _, _, _ = run_pipeline("v", frames=frames, detector=_TwoPeople(), sample_fps=2.0)
        assert state["detections_summary"]["total_detections"] == 4

    def test_stub_detector_gives_zero_people(self, tmp_path):
        frame = _make_frame(tmp_path, 0)
        state, _, _, _, _ = run_pipeline("v", frames=[frame], sample_fps=2.0)
    ...
    def test_empty_frames_gives_zero_summary(self):
        state, det, _, _, _ = run_pipeline("v", frames=[], sample_fps=2.0)
    ...
    def test_state_has_required_top_level_keys(self, tmp_path):
        frame = _make_frame(tmp_path, 0)
        state, _, _, _, _ = run_pipeline("v", frames=[frame], detector=_OnePerson(), sample_fps=2.0)
    ...
    def test_missing_frame_file_produces_empty_detections(self, tmp_path):
        ...
        state, det, _, _, _ = run_pipeline(
            "v", frames=[meta], detector=_OnePerson(), sample_fps=2.0
        )
