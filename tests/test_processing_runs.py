"""Tests for ProcessingRun creation and worker integration."""
...
def _make_mock_db(tmp_path, *, video_id: int = 1, job_id: int = 1, run_id: int = 10):
    """Return a mock async DB session with a fake ProcessingRun attached to the job."""
    ...
    return mock_db, fake_video, fake_job, fake_run, added_objects
...
# ---------------------------------------------------------------------------
# ProcessingRun status transitions
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_processing_run_marked_running_on_start(tmp_path):
    """Worker sets ProcessingRun.status = running when it starts."""
    mock_db, _, _, fake_run, _ = _make_mock_db(tmp_path)
...
@pytest.mark.asyncio
async def test_processing_run_marked_completed_on_success(tmp_path):
    """Worker sets ProcessingRun.status = completed when the job succeeds."""
    mock_db, _, _, fake_run, _ = _make_mock_db(tmp_path)
...
@pytest.mark.asyncio
async def test_processing_run_marked_error_on_failure(tmp_path):
    """Worker sets ProcessingRun.status = error when the job fails."""
    mock_db, _, _, fake_run, _ = _make_mock_db(tmp_path)
    ...
    assert "pipeline exploded" in fake_run.error


@pytest.mark.asyncio
async def test_processing_run_error_recorded_on_failure(tmp_path):
    """Worker stores the exception message in ProcessingRun.error on failure."""
    mock_db, _, _, fake_run, _ = _make_mock_db(tmp_path)
    ...
    with (
        ...
        patch(
            "apps.worker.jobs.process_video.run_pipeline",
            side_effect=RuntimeError("some error detail"),
        ),
        patch("apps.worker.jobs.process_video.settings") as mock_settings,
    ):
        _patch_settings(mock_settings, tmp_path)
    ...
    assert "some error detail" in fake_run.error


# ---------------------------------------------------------------------------
# Artifact rows are linked to the run
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_artifact_rows_linked_to_processing_run(tmp_path):
    """All artifact rows created by the worker carry the processing_run_id."""
    mock_db, _, _, fake_run, added_objects = _make_mock_db(tmp_path)
    ...
    for artifact in artifact_rows:
        assert artifact.processing_run_id == fake_run.id, (
            f"Artifact type={artifact.type!r} has processing_run_id={artifact.processing_run_id!r}"
            f" but expected {fake_run.id}"
        )
